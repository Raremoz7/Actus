...
// Executa `fn` quando o container estiver pronto. Na web o boot é síncrono
// (localStorage) e o dispatch do splash pode disparar ANTES do navigationRef
// ficar pronto — sem isso a navegação inicial seria perdida. No nativo, quando
// já está pronto, roda no ato (sem atraso).
function runWhenReady(fn: () => void, attempts = 60): void {
  if (navigationRef.isReady()) {
    fn();
    return;
  }
  if (attempts <= 0) return;
  setTimeout(() => runWhenReady(fn, attempts - 1), 25);
}
