...
export default function CorpoScreen() {
  ...
  async function save() {
    ...
    const cm = altura.trim() === '' ? null : parseDecimal(altura);

    if (peso.trim() !== '' && (kg === null || kg < 20 || kg > 400)) {
      setPesoError('Peso entre 20 e 400 kg.');
      return;
    }
    if (altura.trim() !== '' && (cm === null || cm < 100 || cm > 250)) {
      setAlturaError('Altura entre 100 e 250 cm.');
      return;
    }

    if (cm !== null) {
      await saveStudentAnswers(user.id, { altura_cm: Math.round(cm) });
    }
    if (kg !== null) {
      patchMe.mutate(
        { body_weight_kg: kg },
