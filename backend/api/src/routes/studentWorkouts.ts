...
router.get("/", async (req, res) => {
  ...
  const active =
    typeof activeQ === "string" ? (activeQ === "true" ? true : activeQ === "false" ? false : null) : null;
  ...
  try {
    ...
    const student_workouts = out.rows.map((r: any) => ({
      id: r.id,
...
router.post("/", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      if (!workoutQ.rowCount) return { ok: false as const, error: "workout_not_found" as const };
      ...
      return { ok: true as const, student_workout_id: ins.rows[0]?.id ?? id };
    });
...
router.patch("/:student_workout_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const rowQ = await client.query<{ id: string; workout_id: string }>(
        `
      ...
      const after = await client.query(
        `select id, student_id, workout_id, weekdays, start_date, end_date, display_order, is_active from public.student_workouts where id = $1`,
        [swId],
      );
    ...
    if (!out.ok) {
      if (out.error === "only_personal" || out.error === "student_not_linked") return res.status(403).json({ error: out.error });
