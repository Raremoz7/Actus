...
router.get("/", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const q = await client.query<{ id: string; name: string; created_at: any }>(
        `select id, name, created_at from public.diet_templates where owner_nutritionist_id = $1 order by created_at desc limit 500`,
        [nutritionistId],
      );
...
router.get("/:diet_template_id", async (req, res) => {
  ...
  if (!z.string().uuid().safeParse(id).success) return res.status(400).json({ error: "invalid_params" });
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const q = await client.query<{ id: string; name: string; body: unknown; created_at: any }>(
        `select id, name, body, created_at from public.diet_templates where id = $1 and owner_nutritionist_id = $2`,
        [id, nutritionistId],
      );
      const row = q.rows[0];
    ...
    if (!out.ok) {
      if (out.error === "only_nutritionist") return res.status(403).json({ error: out.error });
...
router.post("/", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      return { ok: true as const, diet_template_id: q.rows[0]?.id ?? id };
    });
...
router.patch("/:diet_template_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const q = await client.query<{ id: string; name: string; body: unknown; created_at: any }>(
        `select id, name, body, created_at from public.diet_templates where id = $1`,
        [id],
      );
      const row = q.rows[0]!;
      return {
