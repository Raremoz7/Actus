...
router.post("/", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      if (tipo !== "nutricionista") return { ok: false as const, error: "only_nutritionist" as const };
      ...
      if (!tmplQ.rowCount) return { ok: false as const, error: "diet_template_not_found" as const };
      ...
      return { ok: true as const, student_diet_id: ins.rows[0]?.id ?? id };
    });
...
router.patch("/:student_diet_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      const meQ = await client.query<{ tipo: string }>(`select tipo::text as tipo from public.profiles where id = $1`, [nutritionistId]);
      if (meQ.rows[0]?.tipo !== "nutricionista") return { ok: false as const, error: "only_nutritionist" as const };
      ...
      const rowQ = await client.query<{ id: string; diet_template_id: string }>(
        `
      ...
      if (!rowQ.rows[0]) return { ok: false as const, error: "not_found" as const };

      const { diet_template_id, start_date, is_active } = parsed.data;

      if (diet_template_id) {
        const tq = await client.query(
          `select 1 from public.diet_templates where id = $1 and owner_nutritionist_id = $2`,
          [diet_template_id, nutritionistId],
        );
        if (!tq.rowCount) return { ok: false as const, error: "diet_template_not_found" as const };
      }
      ...
      const after = await client.query(
        `select id, student_id, diet_template_id, start_date, is_active from public.student_diets where id = $1`,
        [sdId],
      );
      return { ok: true as const, row: after.rows[0]! };
    });

    if (!out.ok) {
      if (out.error === "only_nutritionist" || out.error === "student_not_linked") return res.status(403).json({ error: out.error });
      if (out.error === "diet_template_not_found") return res.status(403).json({ error: out.error });
