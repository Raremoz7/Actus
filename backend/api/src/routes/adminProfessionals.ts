...
router.post("/", async (req, res) => {
  ...
  const { email, password, full_name, professional_role, cpf, birth_date, phone, gender, policy_version, must_change_password } =
    parsed.data;
...
router.patch("/:user_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      const prof = await client.query<{ tipo: string }>(`select tipo::text as tipo from public.profiles where id = $1 for update`, [targetId]);
      const tipo = prof.rows[0]?.tipo ?? null;
      if (!tipo || (tipo !== "personal" && tipo !== "nutricionista")) {
        return { ok: false as const, error: "target_not_professional" as const };
      }

      if (tipo === "personal" && (b.crn_number !== undefined || b.crn_expires_at !== undefined)) {
        return { ok: false as const, error: "invalid_professional_fields" as const };
      }
      ...
      const ubiFields: string[] = [];
      const ubiVals: unknown[] = [];
      let i = 1;
      ...
      if (ubiFields.length) {
        ...
        if (!info.rowCount) return { ok: false as const, error: "user_basic_info_missing" as const };
        await client.query(`update public.user_basic_info set ${ubiFields.join(", ")} where user_id = $${i}`, ubiVals);
      }

      const piSets: string[] = [];
      const piVals: unknown[] = [];
      let j = 1;
      ...
      if (piSets.length) {
        ...
        await client.query(
          `update public.professional_info set ${piSets.join(", ")}, updated_at = now() where user_id = $${j}`,
          piVals,
        );
      }
      ...
      return { ok: true as const, id: targetId, email: emailRow.rows[0]!.email, tipo: tipoRow.rows[0]!.tipo };
    });

    if (!out.ok) {
      if (out.error === "target_not_professional") return res.status(404).json({ error: "professional_not_found" });
      if (out.error === "invalid_professional_fields") return res.status(400).json({ error: out.error });
