...
export function isoWeekdayFromDateOnly(dateStr: string): number {
...
router.get("/check-ins", async (req, res) => {
  ...
  try {
    const check_ins = await withTx((client) =>
      queryCheckInsForStudent(client, studentId, { from, to, limit }),
    );
...
router.get("/diets/:student_diet_id", async (req, res) => {
  ...
  if (!z.string().uuid().safeParse(sdId).success) return res.status(400).json({ error: "invalid_params" });
  try {
    ...
    if (!row) return res.status(404).json({ error: "student_diet_not_found" });
    return res.json({
      id: row.id,
...
router.get("/workouts/sessions/:session_id", async (req, res) => {
  ...
  if (!z.string().uuid().safeParse(sessionId).success) return res.status(400).json({ error: "invalid_params" });
  try {
...
router.patch("/workouts/sessions/:session_id/exercises/:workout_exercise_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      const sQ = await client.query<{ status: string }>(
        `
      ...
      if (!st) return { ok: false as const, error: "session_not_found" as const };
      if (st !== "in_progress") return { ok: false as const, error: "session_not_editable" as const };

      const exQ = await client.query<{ sid: string }>(
        `
        select se.id as sid
      ...
      if (!exQ.rowCount) return { ok: false as const, error: "session_exercise_not_found" as const };

      await client.query(
        `update public.session_exercises set completed = true, completed_at = now() where session_id = $1 and workout_exercise_id = $2`,
    ...
    if (!out.ok) {
      if (out.error === "session_not_found") return res.status(404).json({ error: out.error });
      if (out.error === "session_not_editable") return res.status(409).json({ error: out.error });
...
router.patch("/workouts/sessions/:session_id", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      const sQ = await client.query<{ status: string }>(
        `select status::text as status from public.workout_sessions where id = $1 and student_id = $2 for update`,
        [sessionId, studentId],
      );
      const cur = sQ.rows[0]?.status;
      if (!cur) return { ok: false as const, error: "session_not_found" as const };

      if (nextStatus === "in_progress") {
        if (cur !== "abandoned") return { ok: false as const, error: "invalid_session_transition" as const };
        await client.query(
          `update public.workout_sessions set status = 'in_progress'::public.workout_session_status, completed_at = null, updated_at = now() where id = $1`,
          [sessionId],
        );
        return { ok: true as const };
      }
      ...
      if (nextStatus === "abandoned") {
        if (cur !== "in_progress") return { ok: false as const, error: "invalid_session_transition" as const };
        await client.query(
          `update public.workout_sessions set status = 'abandoned'::public.workout_session_status, updated_at = now() where id = $1`,
          [sessionId],
        );
        return { ok: true as const };
      }
      ...
      if (nextStatus === "completed") {
        ...
        const pending = await client.query(
          `select 1 from public.session_exercises where session_id = $1 and completed = false limit 1`,
          [sessionId],
        );
        if (pending.rowCount && pending.rowCount > 0) {
          return { ok: false as const, error: "session_exercises_incomplete" as const };
        }
        await client.query(
          `update public.workout_sessions set status = 'completed'::public.workout_session_status, completed_at = now(), updated_at = now() where id = $1`,
          [sessionId],
        );
        return { ok: true as const };
      }

      return { ok: false as const, error: "invalid_session_transition" as const };
    });
...
router.post("/workouts/sessions/:session_id/start", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      const st = sQ.rows[0]?.status;
      if (!st) return { ok: false as const, error: "session_not_found" as const };
      if (st !== "in_progress") return { ok: false as const, error: "session_not_startable" as const };
      await client.query(
        `update public.workout_sessions set started_at = now(), updated_at = now() where id = $1`,
        [sessionId],
      );
      return { ok: true as const };
    });
...
router.post("/workouts/sessions/:session_id/finish", async (req, res) => {
  ...
  try {
    const summaryMeta = await withTx(async (client) => {
      ...
      const row = sQ.rows[0];
      if (!row) return { ok: false as const, error: "session_not_found" as const };

      const terminal = row.status === "completed" || row.status === "completed_partial";
      if (!terminal) {
        if (row.status !== "in_progress") return { ok: false as const, error: "session_not_finishable" as const };
      ...
      let exQ: {
        rows: { id: string; name_snapshot: string; muscle_group: string | null }[];
      };
      try {
        exQ = await client.query<{ id: string; name_snapshot: string; muscle_group: string | null }>(
          `
        ...
        const exQ2 = await client.query<{ id: string; name_snapshot: string }>(
          `
        ...
        exQ = {
          rows: exQ2.rows.map((r) => ({ ...r, muscle_group: null as string | null })),
        };
      }
  ...
  try {
    const out = await withTx(async (client) => {
      ...
      for (const s of parsed.data.sets) {
        if (idx.has(s.set_index)) return { ok: false as const, error: "duplicate_set_index" as const };
        idx.add(s.set_index);
      }
    ...
    if (!out.ok) {
      if (out.error === "session_not_found") return res.status(404).json({ error: out.error });
      if (out.error === "session_not_editable") return res.status(409).json({ error: out.error });
      if (out.error === "duplicate_set_index") return res.status(400).json({ error: out.error });
      return res.status(404).json({ error: out.error });
    }
    const payload = await withTx((client) => loadSessionPayloadTx(client, studentId, sessionId));
    if (!payload) return res.status(404).json({ error: "workout_session_not_found" });
...
router.post("/check-ins", async (req, res) => {
  ...
  try {
    const result = await withTx(async (client) => {
      const d =
        parsed.data.check_in_date?.slice(0, 10) ?? (await studentLocalDateString(client, studentId));
      let workoutSessionId = parsed.data.workout_session_id ?? null;
      if (workoutSessionId != null) {
        const v = await client.query(
          `select 1 from public.workout_sessions where id = $1 and student_id = $2 limit 1`,
          [workoutSessionId, studentId],
        );
        if (!v.rowCount) {
          return { ok: false as const, error: "workout_session_not_found" as const };
        }
      }
...
router.get("/workouts", async (req, res) => {
  ...
  const isActive =
    typeof isActiveQ === "string" ? (isActiveQ === "true" ? true : isActiveQ === "false" ? false : null) : null;
  ...
  try {
    ...
    const items = assignments.map((r: any) => ({
      id: r.id,
...
router.get("/workouts/:student_workout_id", async (req, res) => {
  ...
  if (!z.string().uuid().safeParse(swId).success) return res.status(400).json({ error: "invalid_params" });
  ...
  try {
    const payload = await withTx(async (client) => {
      ...
      try {
        ...
        exQ = {
          rows: exQ2.rows.map((e) => ({ ...e, muscle_group: null as string | null })),
        };
      }

      const sessions = await client.query<{
        id: string;
        scheduled_for_date: any;
        status: string;
        started_at: any;
        completed_at: any;
      }>(
    ...
    if (!payload) return res.status(404).json({ error: "student_workout_not_found" });
    return res.json(payload);
  } catch (e: unknown) {
    return sendInternalError(res, e);
  }
...
router.post("/workouts/:student_workout_id/sessions", async (req, res) => {
  ...
  if (!Number.isFinite(dow)) return res.status(400).json({ error: "invalid_scheduled_date" });
  ...
  try {
    const payload = await withTx(async (client) => {
      const swQ = await client.query<{
        ...
        `select id, workout_id, weekdays, is_active from public.student_workouts where id = $1 and student_id = $2 for update`,
        [swId, studentId],
      );
      const sw = swQ.rows[0];
      if (!sw) return { ok: false as const, error: "student_workout_not_found" as const };
      if (!sw.is_active) return { ok: false as const, error: "student_workout_inactive" as const };
      ...
      const existing = await client.query<{ id: string }>(
        `select id from public.workout_sessions where student_workout_id = $1 and scheduled_for_date = $2::date`,
        [swId, scheduledFor],
      );
      ...
      if (!sessionId) {
        ...
        const we = await client.query<{ id: string }>(
          `select id from public.workout_exercises where workout_id = $1 order by position asc`,
          [sw.workout_id],
        );
      ...
      return { ok: true as const, body: { ...body, schedule_hint: scheduleHint } };
    });
    ...
    if (msg.includes("workout_sessions_unique_day") || msg.includes("unique constraint")) {
      const retry = await withTx(async (client) => {
        ...
        const swQ = await client.query<{ workout_id: string; weekdays: number[] }>(
          `select workout_id, weekdays from public.student_workouts where id = $1 and student_id = $2`,
          [swId, studentId],
        );
...
async function ensureSessionExercises(client: PoolClient, sessionId: string, workoutId: string): Promise<void> {
  const we = await client.query<{ id: string }>(
    `select id from public.workout_exercises where workout_id = $1 order by position asc`,
    [workoutId],
  );
...
async function loadSessionPayload(studentId: string, sessionId: string) {
  return withTx((client) => loadSessionPayloadTx(client, studentId, sessionId));
}

async function setWorkoutSessionCompletedPartial(client: PoolClient, sessionId: string): Promise<void> {
  try {
    await client.query(
      `update public.workout_sessions set status = 'completed_partial'::public.workout_session_status, completed_at = now(), updated_at = now() where id = $1`,
      [sessionId],
    );
  } catch (err) {
    if (isInvalidEnumValue(err, "completed_partial")) {
      const e = new Error(
        "O enum workout_session_status não inclui completed_partial. Aplique as migrações do repositório (ex.: supabase db push).",
      );
...
async function loadSessionPayloadTxModern(client: PoolClient, studentId: string, sessionId: string) {
  const sQ = await client.query<{
    id: string;
    student_workout_id: string;
    scheduled_for_date: any;
  ...
  const setsByExercise = new Map<
    string,
    { set_index: number; weight_kg: number | null; reps_done: number | null; rest_seconds_actual: number | null }[]
  >();
...
async function loadSessionPayloadTxLegacy(client: PoolClient, studentId: string, sessionId: string) {
  const sQ = await client.query<{
    id: string;
    student_workout_id: string;
    scheduled_for_date: any;
  ...
  return {
    ...
    exercises: exQ.rows.map((e) => ({
      ...
      notes: e.notes,
      muscle_group: null,
      sets_logged: [] as { set_index: number; weight_kg: number | null; reps_done: number | null; rest_seconds_actual: number | null }[],
    })),
  };
}

async function loadSessionPayloadTx(client: PoolClient, studentId: string, sessionId: string) {
  try {
