...
async function resolveWeekMonday(
  client: import("pg").PoolClient,
  studentId: string,
  weekStartParam: string | undefined,
): Promise<{ week_monday: string; today: string; timezone: string } | null> {
  ...
  const todayR = await client.query<{ d: unknown }>(
    `select (timezone($1::text, now()))::date as d`,
    [timezone],
  );
  const rawD = todayR.rows[0]?.d;
  const today =
    rawD instanceof Date ? rawD.toISOString().slice(0, 10) : String(rawD ?? "").slice(0, 10);

  if (weekStartParam != null) {
    return { week_monday: weekStartParam, today, timezone };
  }

  const r = await client.query<{ week_monday: unknown }>(
    `
    with loc as (
  ...
  const week_monday =
    rawM instanceof Date ? rawM.toISOString().slice(0, 10) : String(rawM ?? "").slice(0, 10);
  if (!week_monday) return null;
...
router.get("/gamification/weekly-overview", async (req, res) => {
  ...
  try {
    const payload = await withTx(async (client) => {
      ...
      if (!resolved) {
        return { ok: false as const, error: "profile_not_found" as const };
      }
      ...
      const days: {
        ...
        date: string;
        completed: boolean;
        sources: ("check_in" | "workout")[];
      }[] = [];
      for (let i = 0; i < 7; i++) {
        const cal = addDaysIso(resolved.week_monday, i);
        const ci = await client.query(
          `select 1 from public.check_ins c where c.student_id = $1 and c.check_in_date = $2::date limit 1`,
          [studentId, cal],
        );
        ...
        const sources: ("check_in" | "workout")[] = [];
        if (has_check_in) sources.push("check_in");
        if (has_workout) sources.push("workout");
        days.push({
