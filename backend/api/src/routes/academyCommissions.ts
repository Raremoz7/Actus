  ...
  try {
    const out = await withTx(async (client) => {
      const q = await client.query<{
        ...
        `select id, instructor_user_id, rule_type::text as rule_type, percent, amount_cents, currency,
                effective_from, effective_to
         from public.academy_commission_rules
         where academy_id = $1 and active = true
         order by instructor_user_id asc`,
        [academyId],
      );
      return {
        rules: q.rows.map((r) => ({
      ...
      if (instructorUserId) {
        const ok = await client.query(
          `select 1 from public.academy_members
           where academy_id = $1 and user_id = $2 and role = 'instructor' and status = 'active'`,
          [academyId, instructorUserId],
        );
        if (!ok.rowCount) return { ok: false as const };
        await client.query(
          `update public.academy_commission_rules set active = false, effective_to = $1::date, updated_at = now()
           where academy_id = $2 and active = true and instructor_user_id = $3`,
          [effectiveFrom, academyId, instructorUserId],
        );
      } else {
        await client.query(
          `update public.academy_commission_rules set active = false, effective_to = $1::date, updated_at = now()
           where academy_id = $2 and active = true and instructor_user_id is null`,
          [effectiveFrom, academyId],
        );
      }
      const id = uuid();
      await client.query(
        `insert into public.academy_commission_rules
           (id, academy_id, instructor_user_id, rule_type, percent, amount_cents, effective_from, active, created_by)
         values ($1, $2, $3, $4, $5, $6, $7::date, true, $8)`,
        [id, academyId, instructorUserId, rule_type, percent ?? null, amount_cents ?? null, effectiveFrom, actorId],
      );
      return { ok: true as const, id };
    });
    if (!out.ok) return res.status(404).json({ error: "instructor_not_found" });
...
router.get("/base", async (req, res) => {
  ...
  if (!period.success) return res.status(400).json({ error: "invalid_query" });
  ...
  try {
    const out = await withTx(async (client) => {
      const q = await client.query<{
        ...
        `select e.id, e.instructor_user_id, ip.display_name as instructor_name,
  ...
  try {
    const out = await withTx(async (client) => {
      const ok = await client.query(
        `select 1 from public.academy_members
         where academy_id = $1 and user_id = $2 and role = 'instructor' and status = 'active'`,
        [academyId, instructor_user_id],
      );
      if (!ok.rowCount) return { ok: false as const };
      const id = uuid();
      await client.query(
        `insert into public.academy_revenue_entries
           (id, academy_id, instructor_user_id, subject_type, subject_id, period_month, amount_cents, source, note, created_by)
         values ($1, $2, $3, $4, $5, $6::date, $7, 'manual', $8, $9)`,
        [id, academyId, instructor_user_id, subject_type, subject_id ?? null, monthStart, amount_cents, note ?? null, actorId],
      );
      return { ok: true as const, id };
    });
    if (!out.ok) return res.status(404).json({ error: "instructor_not_found" });
...
router.delete("/base/:entry_id", async (req, res) => {
  ...
  if (!z.string().uuid().safeParse(entryId).success) return res.status(400).json({ error: "invalid_params" });
  try {
    const out = await withTx(async (client) => {
      const r = await client.query(
        `delete from public.academy_revenue_entries where id = $1 and academy_id = $2 and source = 'manual'`,
        [entryId, academyId],
      );
      return { changed: r.rowCount ?? 0 };
    });
    if (!out.changed) return res.status(404).json({ error: "entry_not_found" });
...
router.get("/report", async (req, res) => {
  ...
  try {
    const out = await withTx(async (client) => {
      const instructorsQ = await client.query<{ user_id: string; display_name: string | null }>(
      ...
      const studentsQ = await client.query<{ student_id: string; instructor_user_id: string }>(
      ...
      const rulesQ = await client.query<{
        ...
        `select instructor_user_id, rule_type::text as rule_type, percent, amount_cents, effective_from, effective_to, active
         from public.academy_commission_rules where academy_id = $1`,
        [academyId],
      );
      const entriesQ = await client.query<{ instructor_user_id: string; amount_cents: string | number; source: string }>(
        `select instructor_user_id, amount_cents, source::text as source
         from public.academy_revenue_entries where academy_id = $1 and period_month = $2::date`,
        [academyId, monthStart],
      );
      const payoutsQ = await client.query<{ instructor_user_id: string; status: string }>(
        `select instructor_user_id, status from public.academy_commission_payouts
         where academy_id = $1 and period_month = $2::date`,
        [academyId, monthStart],
      );

      const studentCount = new Map<string, number>();
      for (const s of studentsQ.rows) studentCount.set(s.instructor_user_id, (studentCount.get(s.instructor_user_id) ?? 0) + 1);

      const instructors: InstructorInput[] = instructorsQ.rows.map((r) => ({
        user_id: r.user_id,
        display_name: r.display_name,
        student_count: studentCount.get(r.user_id) ?? 0,
      }));
      const rules: CommissionRuleInput[] = rulesQ.rows.map((r) => ({
        instructor_user_id: r.instructor_user_id,
        rule_type: r.rule_type as CommissionRuleInput["rule_type"],
        percent: r.percent == null ? null : Number(r.percent),
        amount_cents: r.amount_cents == null ? null : Number(r.amount_cents),
        effective_from: toDateOnly(r.effective_from),
        effective_to: r.effective_to == null ? null : toDateOnly(r.effective_to),
      ...
      const entries: RevenueEntryInput[] = entriesQ.rows.map((r) => ({
        instructor_user_id: r.instructor_user_id,
        amount_cents: Number(r.amount_cents),
        source: r.source as RevenueEntryInput["source"],
      }));
      const payouts: PayoutInput[] = payoutsQ.rows.map((r) => ({
        instructor_user_id: r.instructor_user_id,
        status: r.status as PayoutInput["status"],
      }));
...
router.post("/payouts", async (req, res) => {
  ...
  try {
    await withTx(async (client) => {
      await client.query(
        `insert into public.academy_commission_payouts
        ...
        [uuid(), academyId, instructor_user_id, monthStart, status, amount_cents, actorId],
      );
    });
