 ...
 */
export function pickMetForSession(hasLoadData: boolean): number {
  return hasLoadData ? 6 : 3.5;
}

/** kcal ≈ MET × peso(kg) × horas (fórmula usual ACSM). */
export function caloriesFromMet(met: number, bodyWeightKg: number, durationMs: number): number {
  if (!Number.isFinite(met) || !Number.isFinite(bodyWeightKg) || bodyWeightKg <= 0 || durationMs <= 0) return 0;
