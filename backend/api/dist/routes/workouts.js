...
router.get("/:workout_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            ...
            if (meQ.rows[0]?.tipo !== "personal")
                return { ok: false, error: "only_personal" };
            const wQ = await client.query(`select id, name, notes, created_at from public.workouts where id = $1 and owner_personal_id = $2`, [workoutId, personalId]);
            const w = wQ.rows[0];
            if (!w)
    ...
    try {
        ...
        `, [workoutId, personalId, name, notes ?? null]);
            ...
            if (!insertedWorkoutId)
                throw new Error("failed_to_create_workout");
            const positions = new Set();
            for (const ex of exercises) {
                if (positions.has(ex.position))
                    return { ok: false, error: "duplicate_exercise_position" };
          ...
          `, [
            ...
            return { ok: true, workout_id: insertedWorkoutId };
        });
        if (!out.ok)
            return res.status(403).json({ error: out.error });
...
router.patch("/:workout_id", async (req, res) => {
    ...
    try {
        ...
        if (!out.ok) {
            if (out.error === "only_personal")
                return res.status(403).json({ error: out.error });
            if (out.error === "duplicate_exercise_position")
                return res.status(400).json({ error: out.error });
            return res.status(404).json({ error: "workout_not_found" });
        }
        const detail = await withTx(async (client) => {
            const wQ = await client.query(`select id, name, notes, created_at from public.workouts where id = $1`, [workoutId]);
            const w = wQ.rows[0];
            const exQ = await client.query(`select id, position, wger_exercise_id, exercise_id, name_snapshot, sets, reps, rest_seconds, notes, muscle_group from public.workout_exercises where workout_id = $1 order by position`, [workoutId]);
            return {
                id: w.id,
                name: w.name,
                notes: w.notes,
