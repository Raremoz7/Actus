...
router.get("/badges/unseen", async (req, res) => {
    ...
    try {
        const body = await withTx(async (client) => {
            const r = await client.query(`select b.id, b.name, b.description, b.asset_key, sb.earned_at
           from public.student_badges sb
           join public.badges b on b.id = sb.badge_id
          where sb.student_id = $1 and sb.seen_at is null
          order by sb.earned_at`, [studentId]);
            return { badges: r.rows };
        });
        return res.json(body);
    }
...
router.post("/badges/seen", async (req, res) => {
    ...
    try {
        await withTx(async (client) => {
            ...
            await client.query(`update public.student_badges set seen_at = now()
          where student_id = $1 and badge_id in (${placeholders}) and seen_at is null`, [studentId, ...ids]);
        });
