...
import { loadChallengeById, buildRankingRows } from "../services/challengeStats.js";
const router = Router();
...
router.post("/challenges/:challenge_id/accept", async (req, res) => {
    ...
    try {
        const payload = await withTx(async (client) => {
            ...
            const pq = await client.query(`select status::text from public.challenge_participants where challenge_id = $1 and student_id = $2`, [challengeId, studentId]);
    ...
    try {
        const payload = await withTx(async (client) => {
            ...
            if (c.visibility === "private_ranking") {
                return { ok: false, error: "ranking_private" };
            }
            const pq = await client.query(`select status::text from public.challenge_participants where challenge_id = $1 and student_id = $2`, [challengeId, studentId]);
            if (pq.rows[0]?.status !== "active") {
                return { ok: false, error: "forbidden_not_participant" };
            }
            const ranking = await buildRankingRows(client, c, true);
            return { ok: true, visibility: c.visibility, ranking };
        });
        if (!payload.ok) {
            if (payload.error === "not_found")
                return res.status(404).json({ error: payload.error });
            if (payload.error === "ranking_private")
                return res.status(403).json({ error: payload.error });
            if (payload.error === "forbidden_not_participant")
                return res.status(403).json({ error: payload.error });
            return res.status(400).json({ error: "unknown" });
        }
