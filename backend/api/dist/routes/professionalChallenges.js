...
import { loadChallengeForOwner, buildRankingRows, } from "../services/challengeStats.js";
const router = Router();
...
function formatChallengeDate(d) {
    if (d instanceof Date)
        return d.toISOString().slice(0, 10);
    return String(d ?? "").slice(0, 10);
}
async function validatePersonalStudents(client, professionalId, studentIds) {
    const invalid = [];
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        limit 200
        `, [ownerId]);
            return q.rows.map((r) => challengeToJson({
                ...r,
...
router.get("/:challenge_id", async (req, res) => {
    ...
    try {
        ...
        `, [challengeId]);
            ...
            return { ok: true, challenge: challengeToJson(challenge), participants };
        });
        if (!payload.ok)
    ...
    try {
        const payload = await withTx(async (client) => {
            ...
            if (challenge.status === "ended")
                return { ok: false, error: "challenge_ended" };
            const invalid = await validatePersonalStudents(client, ownerId, ids);
            if (invalid.length > 0) {
                return { ok: false, error: "invalid_students", invalid };
            }
            let invited = 0;
            for (const sid of ids) {
                const ex = await client.query(`select status::text from public.challenge_participants where challenge_id = $1 and student_id = $2`, [challengeId, sid]);
          ...
          `, [challengeId, sid]);
            ...
            return { ok: true, invited_count: invited };
        });
        if (!payload.ok) {
            if (payload.error === "not_found")
                return res.status(404).json({ error: payload.error });
            if (payload.error === "challenge_ended")
...
router.get("/:challenge_id/ranking", async (req, res) => {
    ...
    try {
        const payload = await withTx(async (client) => {
            ...
            return { ok: true, visibility: challenge.visibility, ranking };
        });
        if (!payload.ok)
            return res.status(404).json({ error: payload.error });
...
router.get("/:challenge_id/report", async (req, res) => {
    ...
    try {
        ...
        `, [challengeId]);
            ...
            const challengeDaySpan = challenge.ends_on >= challenge.starts_on
                ? Math.max(1, Math.round((new Date(challenge.ends_on + "T12:00:00Z").getTime() - new Date(challenge.starts_on + "T12:00:00Z").getTime()) /
                    (86400 * 1000)) + 1)
                : 1;
            const average_adherence_ratio = active_count > 0 ? sumDays / (active_count * challengeDaySpan) : 0;
            const participants = ranking.map((r) => {
                const streak_broken = challenge.status === "active" &&
                    r.active_days > 0 &&
