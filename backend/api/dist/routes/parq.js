...
function plusYearsLocal(dateStr, years) {
    const [y, m, d] = dateStr.split("-").map(Number);
    const nd = new Date(y + years, m - 1, d);
...
studentParqRouter.post("/", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const meQ = await client.query(`select tipo::text as tipo from public.profiles where id = $1`, [requester]);
            if (meQ.rows[0]?.tipo !== "aluno")
                return { ok: false, error: "only_student" };
...
meParqRouter.get("/", async (req, res) => {
    ...
    try {
        const row = await withTx(async (client) => {
            const q = await client.query(`select ${SELECT_COLS} from public.par_q_responses where student_id = $1`, [studentId]);
...
professionalParqRouter.get("/", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const meQ = await client.query(`select tipo::text as tipo from public.profiles where id = $1`, [professionalId]);
