...
router.post("/", async (req, res) => {
    ...
    try {
        const created = await withTx(async (client) => {
            const id = uuid();
            await client.query(`insert into public.academies (id, name, slug, cnpj, timezone, created_by)
         values ($1, $2, $3, $4, coalesce($5, 'America/Sao_Paulo'), $6)`, [id, name, slug ?? null, cnpj ?? null, timezone ?? null, actorId]);
            return { id };
        });
        ...
        if (msg.includes("academies_slug_key"))
...
router.get("/", async (_req, res) => {
    try {
        const out = await withTx(async (client) => {
            const q = await client.query(`select a.id, a.name, a.slug, a.cnpj, a.timezone, a.status, a.created_at,
...
router.get("/:academy_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const a = await client.query(`select id, name, slug, cnpj, timezone, status, created_at from public.academies where id = $1`, [academyId]);
            const academy = a.rows[0];
            if (!academy)
                return null;
            const m = await client.query(`select am.id as member_id, am.user_id, am.role, am.status,
         ...
         order by am.role asc, am.created_at asc`, [academyId]);
            return {
                academy: {
                    id: academy.id,
                    name: academy.name,
...
router.patch("/:academy_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            ...
            const vals = [];
            let i = 1;
            for (const key of ["name", "slug", "cnpj", "timezone", "status"]) {
                if (b[key] !== undefined) {
                    fields.push(`${key} = $${i++}`);
                    vals.push(b[key]);
                }
...
router.post("/:academy_id/manager", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            ...
            if (!ac.rowCount)
                return { ok: false, error: "academy_not_found" };
            const userId = uuid();
            await client.query(`insert into public.app_users (id, email, password_hash, must_change_password) values ($1, $2, $3, $4)`, [userId, email, password_hash, must_change_password]);
            await client.query(`insert into public.profiles (id, display_name, tipo) values ($1, $2, 'gestor'::public.user_role)`, [userId, full_name]);
            if (birth_date) {
                ...
                await client.query(`insert into public.user_basic_info
             (user_id, full_name, cpf_normalized, cpf_hash, cpf_last4, birth_date, phone, gender)
           values ($1, $2, $3, $4, $5, $6::date, $7, $8::public.user_gender)`, [userId, full_name, cpfNorm, cpfHash, cpfLast4, birth_date, phone ?? null, gender ?? "nao_informar"]);
            }
            else if (phone) {
                await client.query(`update public.profiles set phone = $1 where id = $2`, [phone, userId]);
            }
            await client.query(`insert into public.user_lgpd_consents (id, user_id, policy_version, source) values ($1, $2, $3, 'admin')`, [uuid(), userId, policy_version]);
            await client.query(`insert into public.academy_members (id, academy_id, user_id, role, status, invited_by, joined_at)
         values ($1, $2, $3, 'manager', 'active', $4, now())`, [uuid(), academyId, userId, actorId]);
