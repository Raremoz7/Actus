...
router.post("/register", async (req, res) => {
    ...
    try {
        ...
        `, [inv.id, insertedUserId]);
            return { ok: true, userId: insertedUserId };
        });
        if (!created.ok)
...
router.post("/login", async (req, res) => {
    ...
    const row = await withTx(async (client) => {
        const q = await client.query(`select id, password_hash, must_change_password from public.app_users where email = $1`, [email]);
...
router.post("/refresh", async (req, res) => {
    ...
    try {
        ...
        `, [newId, rt.user_id, nextHash, newExpiresAt.toISOString()]);
            ...
            if (!nextId)
                throw new Error("failed_to_rotate_refresh");
            await client.query(`
...
router.post("/change-password", requireAuth, async (req, res) => {
    ...
    try {
        const ok = await withTx(async (client) => {
            const q = await client.query(`select password_hash from public.app_users where id = $1 for update`, [userId]);
            const row = q.rows[0];
            if (!row)
                return false;
