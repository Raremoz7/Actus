...
router.get("/dashboard", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const instructors = await client.query(`select am.user_id, p.display_name
         ...
         where am.academy_id = $1 and am.role = 'instructor' and am.status = 'active'`, [academyId]);
            const students = await client.query(`select student_id, instructor_user_id from public.academy_students where academy_id = $1`, [academyId]);
            const checkins = await client.query(`select c.student_id, c.check_in_date
         from public.check_ins c
         join public.academy_students s on s.student_id = c.student_id and s.academy_id = $1
         where c.check_in_date >= $2::date`, [academyId, since30]);
            ...
            for (const s of students.rows) {
                if (!studentsByInstructor.has(s.instructor_user_id))
                    studentsByInstructor.set(s.instructor_user_id, new Set());
            ...
            const ranking = instructors.rows
                .map((ins) => {
                const studentIds = studentsByInstructor.get(ins.user_id) ?? new Set();
...
router.get("/members", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            ...
            const q = await client.query(`select am.id as member_id, am.user_id, am.role, am.status, p.display_name, au.email,
         ...
         order by am.role asc, p.display_name asc`, params);
            return {
...
router.post("/members/instructors", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const u = await client.query(`select au.id, p.tipo::text as tipo
         from public.app_users au
         join public.profiles p on p.id = au.id
         where au.email = $1`, [email]);
            const user = u.rows[0];
            if (!user)
                return { ok: false, error: "personal_not_found", status: 404 };
            if (user.tipo !== "personal")
                return { ok: false, error: "not_a_personal", status: 400 };
         ...
         do update set role = 'instructor', status = 'active', joined_at = now(), updated_at = now()`, [uuid(), academyId, user.id, actorId]);
...
router.patch("/members/:user_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const r = await client.query(`update public.academy_members set status = $1, updated_at = now()
         where academy_id = $2 and user_id = $3 and role = 'instructor'`, [status, academyId, userId]);
...
router.delete("/members/:user_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const r = await client.query(`update public.academy_members set status = 'revoked', updated_at = now()
         where academy_id = $1 and user_id = $2 and role = 'instructor'`, [academyId, userId]);
            return { changed: r.rowCount ?? 0 };
        });
        if (!out.changed)
            return res.status(404).json({ error: "member_not_found" });
