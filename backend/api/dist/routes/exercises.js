...
router.get("/", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            ...
            if (muscle) {
                conditions.push(`$${i++} = ANY(primary_muscles)`);
                vals.push(muscle);
            }
            if (machine_type) {
                conditions.push(`machine_type = $${i++}`);
                vals.push(machine_type);
            }
            const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
            const countQ = await client.query(`SELECT count(*)::text AS total FROM public.exercises ${where}`, vals);
            const total = Number(countQ.rows[0]?.total ?? 0);
            vals.push(limit, offset);
            const dataQ = await client.query(`SELECT id, name_pt, name_en, category, equipment, level, mechanic, force,
...
router.get("/:id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const q = await client.query(`SELECT id, name_pt, name_en, category, equipment, level, mechanic, force,
                primary_muscles, secondary_muscles, instructions_pt, image_0_url, image_1_url,
                machine_type
         FROM public.exercises WHERE id = $1`, [id]);
