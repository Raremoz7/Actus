...
router.patch("/:link_id", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const cur = await client.query(`select id, student_id, professional_role::text, status::text from public.student_professional_links where id = $1 for update`, [linkId]);
          ...
          `, [row.student_id, row.professional_role, linkId]);
                if (clash.rowCount)
                    return { ok: false, error: "conflict_active_role" };
            }
            await client.query(`update public.student_professional_links set status = $1::public.link_status where id = $2`, [
                status,
                linkId,
            ]);
            return { ok: true, link_id: linkId, status };
        });
        if (!out.ok) {
            if (out.error === "conflict_active_role")
