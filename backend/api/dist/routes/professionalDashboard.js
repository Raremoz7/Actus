...
async function fetchActiveStudentIds(client, personalId) {
    const q = await client.query(`select spl.student_id
     from public.student_professional_links spl
     where spl.professional_id = $1 and spl.professional_role = 'personal' and spl.status = 'active'`, [personalId]);
    return q.rows.map((r) => r.student_id);
}
async function fetchCheckInsSince(client, personalId, since) {
    const q = await client.query(`select c.student_id, c.check_in_date
     ...
     where c.check_in_date >= $2::date`, [personalId, since]);
    return q.rows.map((r) => ({
...
router.get("/at-risk", async (req, res) => {
    ...
    try {
        const out = await withTx(async (client) => {
            const q = await client.query(`select spl.student_id, p.display_name, au.email,
