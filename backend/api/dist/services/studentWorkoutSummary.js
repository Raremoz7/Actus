...
function maxWeightPerExercise(rows) {
    const m = new Map();
    for (const r of rows) {
        ...
        if (w > cur)
            m.set(r.workout_exercise_id, w);
    }
    return m;
}
function totalVolumeKg(rows) {
    let v = 0;
    for (const r of rows) {
        const w = r.weight_kg == null ? null : Number(r.weight_kg);
        const reps = r.reps_done ?? 0;
        if (w != null && Number.isFinite(w) && reps > 0)
            v += w * reps;
    }
    return Math.round(v);
}
export async function buildWorkoutFinishSummary(client, studentId, sessionId, exerciseMeta) {
