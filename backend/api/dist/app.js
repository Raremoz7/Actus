...
import { studentParqRouter, meParqRouter, professionalParqRouter, professionalParqListRouter, } from "./routes/parq.js";
 ...
 */
function createSwaggerUiTagsSorter() {
    const orderJson = JSON.stringify(OPENAPI_TAGS.map((t) => t.name));
    return new Function("a", "b", `"use strict";
    try {
      var order = ${orderJson};
...
export function createApp() {
    ...
    function swaggerNoStore(_req, res, next) {
        res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate");
        next();
    }
    /**
     ...
     */
    app.use("/docs", swaggerNoStore, swaggerUi.serve, swaggerUi.setup(null, {
